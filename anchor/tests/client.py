...
async def main():
    ...
    program_id = PublicKey("52nm5mLXp6rEu5Qr27Y3u2ycveB43eGkRxABiiMQcrbE")
    ...
    pda_index = 1
    (counter_pubkey, _) = get_pda_address(SEED, program_id, wallet.public_key, pda_index)
    logging.info(f"Counter account pubkey: {counter_pubkey}")
    ...
    async with Program(idl, program_id, anchorpy_provider) as program:
        ...
        if args.type == "INIT":
            txn_signature: TransactionSignature = await program.rpc["initialize"](
                ...
                ctx=Context(
                    accounts={
                        "pda_counter_account": counter_pubkey,
            ...
            ix1: TransactionInstruction = program.instruction["set_counter"](
                ...
                ctx=Context(
                    ...
                    signers=[],  # default signer is the wallet keypair
                ),
            )
